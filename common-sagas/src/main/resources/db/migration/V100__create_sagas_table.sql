-- Saga table for managing distributed transaction state
-- This table stores saga execution state for coordination across microservices
...
-- Index for ensuring a saga is unique per business correlation
CREATE UNIQUE INDEX uq_sagas_type_correlation ON sagas(saga_type, correlation_id);

-- Index for saga state filtering (useful when querying in-progress sagas)
